...
void generate() {
  ...
  int cc = int(random(300, 360)*0.08);
  float bb = 20;
  ...
  for(int j = 0; j < cc; j++){
     for(int i = 0; i < cc; i++){
       ...
       PVector p = desform(bb+(i+0)*ss, bb+(j+0)*ss);
       vertex(p.x, p.y);
       p = desform(bb+(i+1)*ss, bb+(j+0)*ss);
       vertex(p.x, p.y);
       p = desform(bb+(i+1)*ss, bb+(j+1)*ss);
       vertex(p.x, p.y);
       p = desform(bb+(i+0)*ss, bb+(j+1)*ss);
       vertex(p.x, p.y);
     }
  }
  endShape();
  ...
  */
  ...
  for(int i = 0; i < 50; i++){
    fill(rcol());
    float s = constrain(random(TAU)+cos(time*random(0.3))*5, 0, 1);
     circle(width*random(0.2, 0.8), height*random(0.2, 0.8), width*random(0.05, 0.25)*s); 
  }
}
