public class ColorSchemes {
  boolean view = false;
  int selX, selY, selC;
  int colors[][] = {{#FFFCF7, #FDDA02, #EE78AC, #3155A3, #028B88}, {#01AFD8, #009A91, #E46952, #784391, #1B2D53}, {#ADFE03, #FDFEFA, #FC4627, #070708}, {#FF5F57, #FF407B, #FFAFDD, #43CA69, #4B94FB, #EEDE34}};
  PApplet parent;
  ...
  void prev() {
    selY = (selY-1+colors.length)%colors.length;
    selX = selX%colors[selY].length;
  }
  void next() {
    selY = (selY+1)%colors.length;
    selX = selX%colors[selY].length;
  }

  int rcol() {
    return colors[selY][int(random(colors[selY].length))];
  }
