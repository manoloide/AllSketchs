...
void generate() {
  ...
  float scl = random(1, 2);

   ArrayList<Rect> rects = new ArrayList<Rect>();
   rects.add(new Rect(0, 0, width*scl, height*scl));

   int sub = int (random(150));
   for (int i = 0; i < sub; i++) {
     int ind = int(random(rects.size()));
     Rect r = rects.get(ind);
     int dw = int(random(2, random(4)));
     int dh = int(random(2, random(8)));
     float mw = r.w/dw;
     float mh = r.h/dh;
   ...
   for (int i = 0; i < rects.size(); i++) {
     ...
     float ss = min(ww*0.2, hh*0.2);
     float vx = time*random(-1, 1);
     float vy = time*random(-1, 1);
     noStroke();
     fill(rcol());
     translate(xx+cos(vx)*(ww-ss)*0.5, yy+sin(vy)*(hh-ss)*0.5, 0.5);
     sphereDetail(20);
     sphere(ss*0.5);
     for(int j = 0; j < 3; j++){
       stroke(rcol(), random(100, 256));
       noFill();
       float s = ss*random(1, 3);
       ellipse(0, 0, s, s);
     }
     //ellipse(xx+cos(vx)*(ww-ss)*0.5, yy+sin(vy)*(hh-ss)*0.5, ss, ss);
     popMatrix();
   }
...
void back() {
  ArrayList<Rect> rects = new ArrayList<Rect>();
  float sca = 1.8;
  rects.add(new Rect(0, 0, width*sca, height*sca));

  int sub = int (random(12));
  for (int i = 0; i < sub; i++) {
    int ind = int(random(rects.size()));
    Rect r = rects.get(ind);
    int dw = int(random(2, random(4)));
