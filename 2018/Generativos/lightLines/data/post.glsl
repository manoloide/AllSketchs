...
void main(void) {
    ...
    vec3 sum = mix(col, blur(), 0.95);

    float dis = 1.0-pow(clamp(distance(st, vec2(0.5)), 0.0, 1.0), 2.4)*0.9;

    sum = mix(sum, vec3(rand(st*300)), 0.04);
    sum = csb(sum, 1.2, 0.8+dis*0.4, 1.4)*dis;

    sum.r = pow(sum.r, 1.0);
    sum.g = pow(sum.g, 0.92);
    sum.b = pow(sum.b, 1.2);

    //sum *= 0.5+pow(cos(st.x*960.0*1.0), 0.4)*0.5;
