...
void main(void) {
    ...
    vec3 sum = col;//mix(col, blur(), .0);
    
    
    float dis = 1.0-pow(clamp(distance(st, vec2(0.5)), 0.0, 1.0), 2.4)*0.2;
    
    sum = mix(sum, vec3(0.0), dis*0.2);

    sum = mix(sum, vec3(luma+rand(st*8)*0.2-0.2), 0.42*(1-luma));
    sum = csb(sum, 1.1+luma*0.05, 1.42+(1.0-dis)*2.6, 1.1)*dis;
