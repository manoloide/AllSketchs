int seed = int(random(999999));

void setup() {
  size(3250, 3250, P2D);
  smooth(2);
  pixelDensity(2);

  for (int i = 0; i < 10; i++) {
    generate();
    saveImage();
  }
}

void draw() {
}

void keyPressed() {
  if (key == 's') saveImage();
  else {
    seed = int(random(999999));
    generate();
  }
}

void generate() {

  rectMode(CENTER);
  background(rcol());

  float ss = random(40, 320);
  int cc = int(width*1./ss)-1;
  float bb = (width-ss*cc)*0.5;

  int gc = (cc+2)*4;
  float gs = width*1./gc;
  noStroke();
  for (int j = 0; j <= gc; j++) {
    for (int i = 0; i <= gc; i++) {
      float x = i*(0.0+gs);
      float y = j*(0.0+gs);
      noStroke();
      fill(255, 8);
      //rect(x, y, gs*0.90, gs*0.30);
      //rect(x, y, gs*0.30, gs*0.90);
      pushMatrix();
      translate(x, y);
      rotate(PI*0.25);
      rect(0, 0, gs*0.6, gs*0.6);
      popMatrix();
      fill(0, 6);
      rect(x, y, gs*0.25, gs*0.25);
      ellipse(x+gs*0.5, y+gs*0.5, gs*0.2, gs*0.2);
    }
  }

  fill(rcol());
  triLine(0, 0, width, 0, gc);
  triLine(width, 0, width, height, gc);
  triLine(width, height, 0, height, gc);
  triLine(0, height, 0, 0, gc);

  noStroke();
  float ms = ss*0.5;
  for (int j = 0; j < cc; j++) {
    for (int i = 0; i < cc; i++) {
      pushMatrix();
      translate(bb+(i+0.5)*ss, bb+(j+0.5)*ss);
      rotate(HALF_PI*0.5);

      fill(rcol());
      rectShadow(ss*1.25, 140, 0);

      beginShape();
      fill(rcol());
      vertex(-ms, -ms);
      vertex(+ms, -ms);
      fill(rcol());
      vertex(+ms, +ms);
      vertex(-ms, +ms);
      endShape(CLOSE);

      /*
      triLine(-ms, -ms, +ms, -ms, 10);
       triLine(+ms, -ms, +ms, +ms, 10);
       triLine(+ms, +ms, -ms, +ms, 10);
       triLine(-ms, +ms, -ms, -ms, 10);
       */

      beginShape();
      fill(0, 0);
      vertex(0, -ms);
      vertex(0, +ms);
      fill(0, 80);
      vertex(+ms, +ms);
      vertex(+ms, -ms);
      endShape(CLOSE);

      fill(0);
      rectShadow(ss*0.25, 80, 0);

      fill(rcol());
      rect(0, 0, ss*0.2, ss*0.2);
      popMatrix();
    }
  }

  for (int i = 0; i < 100; i++) {
    float x = random(width);
    float y = random(height);
    float s = random(6);
    float ang = random(TWO_PI);
    float amp = random(0.2, 0.3);
    fill(255, 200);
    //starSha(x, y, s*2, ang, amp);
    fill(rcol(), 120);
    star(x, y, s, ang, amp);
  }
}

void star(float x, float y, float s, float ang, float amp) {
  float da = TWO_PI/10;
  float r = s*0.5;
  beginShape();
  for (int i = 0; i < 10; i++) {
    float a = da*i+ang;
    vertex(x+cos(a)*r, y+sin(a)*r);
    vertex(x+cos(a+da*0.5)*r*amp, y+sin(a+da*0.5)*r*amp);
  }
  endShape(CLOSE);
}

void starSha(float x, float y, float s, float ang, float amp) {
  float da = TWO_PI/10;
  float r = s*0.5;
  int col = g.fillColor;
  for (int i = 0; i < 10; i++) {
    float a = da*i+ang;
    beginShape();
    fill(col, 200);
    vertex(x, y);
    fill(col, 0);
    vertex(x+cos(a)*r, y+sin(a)*r);
    vertex(x+cos(a+da*0.5)*r*amp, y+sin(a+da*0.5)*r*amp);
    endShape(CLOSE);

    beginShape();
    fill(col, 60);
    vertex(x, y);
    fill(col, 0);
    vertex(x+cos(a)*r, y+sin(a)*r);
    vertex(x+cos(a-da*0.5)*r*amp, y+sin(a-da*0.5)*r*amp);
    endShape(CLOSE);
  }
}

void rectShadow(float ss, float a1, float a2) {
  float r = ss*0.5*sqrt(2);
  int col = g.fillColor;
  for (int i = 0; i < 4; i++) {
    float ang = HALF_PI*(i+0.5);
    beginShape();
    fill(col, a1);
    vertex(0, 0);
    fill(col, a2);
    vertex(cos(ang)*r, sin(ang)*r);
    vertex(cos(ang+HALF_PI)*r, sin(ang+HALF_PI)*r);
    endShape(CLOSE);
  }
}

void triLine(float x1, float y1, float x2, float y2, int cc) {
  float dist = dist(x1, y1, x2, y2);
  float ang1 = atan2(y2-y1, x2-x1);
  float ang2 = atan2(y2-y1, x2-x1)+HALF_PI;
  float dd = dist/cc;
  for (int i = 0; i < cc; i++) {
    float d1 = i*dd;
    float d2 = (i+1)*dd;
    float dm = (d1+d2)*0.5;
    beginShape();
    vertex(x1+cos(ang1)*d1, y1+sin(ang1)*d1);
    vertex(x1+cos(ang1)*dm+cos(ang2)*dd*0.5, y1+sin(ang1)*dm+sin(ang2)*dd*0.5);
    vertex(x1+cos(ang1)*d2, y1+sin(ang1)*d2);
    endShape(CLOSE);
  }
}

void saveImage() {
  String timestamp = year() + nf(month(), 2) + nf(day(), 2) + "-"  + nf(hour(), 2) + nf(minute(), 2) + nf(second(), 2);
  saveFrame(timestamp+".png");
}

//int colors[] = {#FACD00, #FB4F00, #F277C5, #7D57C6, #00B187, #3DC1CD};
int colors[] = {#F19617, #251207, #15727F, #CEAB81, #BD3E36};
//int colors[] = {#F5BFDF, #FBD872, #F36B7F, #9D87D2, #98B6F2, #1F5CB6};
int rcol() {
  return colors[int(random(colors.length))];
}
int getColor() {
  return getColor(random(colors.length));
}
int getColor(float v) {
  v = abs(v);
  v = v%(colors.length); 
  int c1 = colors[int(v%colors.length)]; 
  int c2 = colors[int((v+1)%colors.length)]; 
  return lerpColor(c1, c2, v%1);
}