...
void generate() {
  ...
  rotateX(random(TAU)*0.1);
  rotateY(random(TAU)*0.1);
  rotateZ(random(TAU)*0.1);
  ...
  int grid = int(random(100, 300));//int(width*1./pow(2, int(random(random(7, 8), 9))));
  float gs = width*2./grid;
  int gc = rcol();
