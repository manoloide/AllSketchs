...
void generate() { 
  ...
  perspective(width*random(0.3, 0.7), height*random(0.3, 0.7), int(random(60, 360)), random(0.01, 0.1), rcol(), rcol());
  perspective(width*random(0.3, 0.7), height*random(0.3, 0.7), int(random(60, 360)), random(0.5, random(0.5, 0.8)), rcol(), rcol());
  ...
  for (int j = 0; j < 20; j++) {
    ...
    float dc = random(0.1)*random(0.1, 1)*random(0.1, 1)*random(0.1, 1);
    ...
    for (int i = 0; i < lar; i++) {
      ...
      noStroke();
      if (random(1) < 0.004) {
        arc2(ax1, ay1, 0, 32, 0, TAU, color(255), 30, 0);
      }
      if (random(1) < 0.0002) {
        stars.add(new PVector(ax1, ay1));
      }
      if (i < lar-2) {
        stroke(255, 250);
        line(ax1, ay1, x1, y1);
    ...
    for (int i = 0; i < stars.size(); i++) {
      ...
      fill(255);
      start(s.x, s.y, ss*0.6, int(random(4, 10)));
    }
  }
...
void start(float x, float y, float s, int c) {
  float a = random(TAU);
  c *= 2;
  float da = TAU/c;
  float r = s*0.5;
  ...
  for (int i = 0; i < c; i++) {
    float ang = a+i*da;
    float rr = r*(1-(i%2)*0.9);
    vertex(x+cos(ang)*rr, y+sin(ang)*rr);
  }
  endShape(CLOSE);
}

void perspective(float cx, float cy, int cc, float amp, int c1, int c2) {
  float da = TWO_PI/cc;
  float r = width*1.42;

  float ic1 = random(colors.length);
  float ic2 = random(colors.length);
  float dc1 = int(random(1, random(1, 10)))*colors.length*1./cc;
  float dc2 = int(random(1, random(1, 10)))*colors.length*1./cc;
  for (int i = 0; i < cc; i++) {
    float a1 = da*i;
    float a2 = da*(i+amp);
    beginShape();
    fill(getColor(ic1+dc1*i));
    vertex(cx, cy);
    fill(getColor(ic2+dc2*i));
