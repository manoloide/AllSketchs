...
void generate() {
  ...
  rects.add(new Rect(-width*0.8, -height*0.8, width*1.6, height*1.6));
  int sub = int(random(200)*random(0.1, 1));
  float max = 20;
  for (int i = 0; i < sub; i++) {
  ...
  for (int i = 0; i < rects.size(); i++) {
    ...
    boolean rndColor = random(1) < 0.4;
    
    for (int z = 0; z < cz; z++) {
      for (int y = 0; y < cy; y++) {
        if (z != 0 &&  z != cz-1 && y != 0 && y != cy-1) continue; 
        if(rndColor) fill(rcol());
        for (int x = 0; x < cx; x++) {
          if(rndColor)if (random(1) < 0.12) fill(rcol());
