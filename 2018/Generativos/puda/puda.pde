...
void generate() {
  ...
  for (int i = 0; i < 0; i++) {
    ...
    float s = width*random(1)*random(1)*random(0.5, 1)*0.2;
    ...
    stroke(rcol(), 40);
    noStroke();
    float amp = random(0.01, 0.2)*random(0.25, 1);
    for (int j = 0; j < 30; j++) {
...
PVector desform(float x, float y) {
  float ang = (float) SimplexNoise.noise(desAng+x*detAng, desAng+y*detAng)*TAU*20.9;
...
int colors[] = {#FEAFCC, #70A7FB, #010101, #BE0117, #FEAFCC, #BE0117};
