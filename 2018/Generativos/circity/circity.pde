    ...
    for (int i = 0; i < sub; i++) {
      ...
      for (int k = 0; k < 4; k++) {
        ...
        if (rnd == 4) {
          ...
          float sss = (ss*man)/cc;
          float ssss = int(sss*random(0.1, 0.9));
          fill(rcol());
          for (int m = 0; m < cc; m++) {
            for (int l = 0; l < cc; l++) {
              rect(xx+sss*(l+0.5-cc*0.5), yy+sss*(m+0.5-cc*0.5), ssss, ssss);
            }
          }
        }
      }
...
int colors[] = {#BA2F1A, #EEA313, #0C7E42, #00134D, #222126, #E5E5E5};
int rcol() {
