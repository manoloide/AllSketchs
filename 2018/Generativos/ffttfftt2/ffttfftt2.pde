...
void generate() { 
  ...
  int cc = int(width/40);
  float ss = width*1./cc;
  ArrayList<Rect> brects = new ArrayList<Rect>();
  brects.add(new Rect(0, 0, width, height));
  int div = int(random(180));
  for (int i = 0; i < div; i++) {
    int ind = int(random(brects.size()));
    Rect r = brects.get(ind);
    int iw = int(r.w/ss);
    int ih = int(r.h/ss);
    if (iw > 1 && ih > 1) {
      ...
      float nw = cw*ss;
      float nh = ch*ss;
      brects.add(new Rect(r.x, r.y, nw, nh));
      brects.add(new Rect(r.x+nw, r.y, r.w-nw, nh));
      brects.add(new Rect(r.x+nw, r.y+nh, r.w-nw, r.h-nh));
      brects.add(new Rect(r.x, r.y+nh, nw, r.h-nh));
      brects.remove(ind);
    }
  }
  for (int i = 0; i < brects.size(); i++) {
    Rect r = brects.get(i);
    fill(lerpColor(rcol(), color(255), random(1)));
  ...
  for (int i = 0; i < rects.size(); i++) {
    ...
    if (random(1) < 0.6) {
      ...
      int x1 = int(r.x+random(0.1, 0.9)*r.w)*2;
      int x2 = int(r.x+random(x1/2-r.x, 0.9*r.w))*2;
      int y1 = int(r.y+random(0.1, 0.9)*r.h)*2;
      int y2 = int(r.y+random(y1/2-r.y, 0.9*r.h))*2;
      if(random(1) < 0.2){
         x1 = int(r.x)*2; x2 = int(r.x+r.w)*2; 
         y1 = int(r.y)*2; y2 = int(r.y+r.h)*2; 
      }
      det = random(0.01);
      des = random(100);
      for (int yy = y1; yy < y2; yy++) {
        for (int xx = x1; xx < x2; xx++) {
          int cl = getColor(noise(des+xx*det, des+yy*det)*colors.length*2);
          set(xx, yy, cl);
        }
      }
    }
  ...
  for (int i = 0; i < 40; i++) {
    ...
    stroke(255, 2);
    arc2(x, y, s*0.6, s, 0, TAU, rcol(), 30, 0);
    fill(250);
