...
void generate() { 
  ...
  for (int i = 0; i < points.size(); i++) {
    ...
    float r = p.z;
    float cc = r*r*PI*random(0.4, 0.5)*random(0.02)*random(1)*random(random(1));
    ...
    noStroke();
    for (int j = 0; j < cc; j++) {
      float mr = random(random(0.2, 1), 1);
      //fill(rcol());
      arc2(p.x, p.y, 0, r*mr, an+j*da, an+(j+1)*da, rcol(), random(255), random(255));
    }
    /*
  ...
  for (int i = 0; i < points.size()*random(0.3, 0.4); i++) {
    ...
    if (random(1) < 0.8) {
      ...
      noStroke();
      fill(rcol(), 4);
      arc2(p1.x, p1.y, dis, dis, a, a+HALF_PI, rcol(), random(255), random(255));
    }
  }
  strokeWeight(1);
}
