...
void draw() {
  ...
  boolean masActive = false;

  //invActive = noiActive = ranActive = false;
  //oscActive = true;

  float velLoad = random(0.006, random(random(random(0.5))));//random(0.0, 1);//random(0.8)*random(1);
  float fade = random(random(1));//random(random(0.95, 1), 1);
  ...
  for (int j = 0; j < ch; j++) {
    for (int i = 0; i < cw; i++) {
      ...
      noStroke();
      fill(240, pow(values[i][j], pwr)*500);
