...
void bb(float x, float y, float z, float w, float h, int ite) {
  ...
  for (int i = 0; i < div; i++) {
    ...
    float m1w = curve((float) noise(100+des+r.x*det, des+r.y*det, -23.6+i+time*vel), 2.4);
    float m1h = curve((float) noise(des+r.x*det, 100+des+r.y*det, +57.6+i+time*vel), 2.4);
    float m2w = 1-m1w;
    float m2h = 1-m1h;
  ...
  for (int i = 0; i < rects.size(); i++) {
    Rect r = rects.get(i);
    fill(getColor(time*random(0.1)+random(20.2)));
