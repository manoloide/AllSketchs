...
void draw() {
  ...
  //background((invertir) ? invertirImage(video) : video);
...
void chequearExplocion(PImage movimiento) {
  for (int i=0; i<posBurbujas.size(); i++) {
    ...
    // Explode the actual bubble according the explosion threshold
...
PImage conseguirMovimiento(PImage video, PImage anterior, boolean invertir) {
  //imagen acual 
  PImage actual = createImage(video.width, video.height, RGB);
  // Create a copy of the actual image to create the movement image
  ...
  for (int x = 0; x < actual.width; x++) {
    for (int y = 0; y< actual.height; y++) {
      ...
      color act  = actual.pixels[index];
      color previous = anterior.pixels[index];
      //calcuala de distancia de colores 
      float diff = dist(red  (act), green(act), blue (act), red  (previous), green(previous), blue (previous));
...
PImage invertirImage (PImage source) {
  ...
  for (int x=0; x<source.width; x++) {
    for (int y=0; y<source.height; y++) {
      // Calculate the inverted X (loc) for the actual X
