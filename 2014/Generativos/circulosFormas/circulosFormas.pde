/*
 -generar varios planos y colores
 */
...
void generar() {
  ...
  int cantplanos = 8;
  for (int j = 0; j < cantplanos; j++) {
    float cx = width/2;//(j*1./cantplanos)*width/6+width/3;
    float cy = height/2;  
    float tam = map(j, 0, cantplanos, -10, width);
      float t = map(j, 0, cantplanos, 10, 160);
    int cant = int(map(j,0, cantplanos, 1000, 300)*map(j,0,cantplanos,2, 0.6));
    for (int i = 0; i < cant; i++) {
      float x = random(width);
      float y = random(height);
      if (dist(x, y, cx, cy) > tam/2+t/2) {
