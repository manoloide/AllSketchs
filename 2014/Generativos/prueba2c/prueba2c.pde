//que se genere apartir de un seed
...
void setup() {
  ...
  circulos.add(new Circulo(width*random(0.2,0.8), height*random(0.2,0.8), 10, 420));
}
...
class Arco {
  ...
  Arco(Circulo c, float x, float y, float dim) {
    ...
    ang1 = random(TWO_PI);
    ang2 = random(TWO_PI);
    col = color(random(255), random(180, 255), random(240, 255), 256-(random(1)*random(200)));
  }
  void act() {
    tiempo--;
    if (tiempo <= 0) cambiar();
  ...
  void cambiar() {
    ...
    ocol = color(random(255), random(180, 255), random(240, 255), 256-(random(1)*random(200)));
