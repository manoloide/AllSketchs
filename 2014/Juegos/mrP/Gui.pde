...
class Gui {
  ...
  void act() {
    ...
    if (visible) {
      if (click && getValor("Esconder menu") == 1 && ((mouseX < px || mouseX > px+w) ||(mouseY < py || mouseY > py+h))) {
        visible = false;
      } 
      float dh = 30;
...
class Pestana {
  ...
  void act(float px, float py) {
    w = gui.w;
    if (gui.click && mouseX >= px && mouseX < px+w &&mouseY > py && mouseY <= py+20) {
...
class MenuPressets extends Elemento {
  ...
  void act(float cx, float cy) {
    ...
    if (scroll) {
      float px = x+w-10;
      float py = y;
      if (gui.click && ind > 0 && mouseX >= px && mouseX < px+20 && mouseY >= py && mouseY < y+20) {
        ind--;
      }
      if (gui.click && maxval < pressets.size() && mouseX >= px && mouseX < px+20 && mouseY >= py+h-20 && mouseY < y+h) {
        ind++;
      }
    }
