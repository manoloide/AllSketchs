...
void setup() {
  ...
  strokeWeight(1.2);
  img = loadImage("http://g.cdn.mersap.com/literatura-contemporanea/files/2010/12/julio-cortazar.jpg");
  camara = new Camara();
  paleta = new color[3];
  paleta[0] = #F9F9F9;
  paleta[1] = #FC9545;
...
void draw() {
  for (int i = 0; i < 10; i++) {
    //Texto aux = new Texto(palabras[int(random(palabras.length))], random(img.width), random(img.height), 0);//random(-200, 200));
    //aux.ry = (TWO_PI/4)*int(random(4));
    //textos.add(aux);
  }
  translate(width/2-img.width, height/2-img.height);

  // background(#26212C);
  //camara.act();
  for (int i = 0; i < 80; i++) {
    String tex =  palabras[int(random(palabras.length))];
    pushMatrix();
    int x = int(random(img.width*2));
    int y = int(random(img.height*2));
    translate(x, y, 0);
    //rotateZ(random(TWO_PI));
    float tam = random(6, 20);
    textSize(tam);
    fill(img.get(int(x/2), int(y/2)));//,10+9*(20-tam));
    text(tex, -textWidth(tex)/2, 0);
    popMatrix();
  }
...
class Texto {
  ...
  Texto(String text, float x, float y, float z) {
    ...
    rz = random(TWO_PI);
    tam = random(6, 12);
    col = img.get(int(x), int(y));//paleta[int(random(1)*random(3))];
  }
  void act() {
    dibujar();
  }
