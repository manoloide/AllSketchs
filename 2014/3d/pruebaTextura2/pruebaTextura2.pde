...
void draw() {
  ...
  for (int i = 0; i < 80; i++) {
    ...
    float sep = 500;
    float des = random(1);
    float rot = random(1)*0.01;
    translate(random(-sep, sep), random(sep, sep*2.5)-frameCount*des, random(-sep, sep));
...
PImage crearTextura() {
  ...
  aux.beginDraw();
  aux.background(255);//rcol());
  int r = int(random(1));
  if (r == 0) {
    aux.stroke(rcol());
    int esp = int(random(8, 24));
    aux.strokeWeight(esp*random(1.2, 1.4));
