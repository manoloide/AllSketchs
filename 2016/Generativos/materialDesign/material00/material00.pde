...
void generate() {
  ...
  int p1 = int(random(MaterialColors.getNumPalettes()-5));
  int p2 = int(random(MaterialColors.getNumPalettes()-5));

  noStroke();
  for (int i = 0; i < 1000; i++) {
    float x = random(width);
    float y = random(height);
    float s = random(20)*pow(2, random(4));
    ...
    int p = (random(1) < 0.5)? p1 : p2;
    int c = int(random(MaterialColors.getNumColor(p))); 
    fill(MaterialColors.getColor(p, c));
    ellipse(x, y, s, s);
  }
...
static class MaterialColors {
  ...
  static int getNumPalettes() {
    return palettes.length;
  }

  static int getNumColor(int p) {
    p = p % getNumPalettes();
    return palettes[p].length;
  }

  static int getColor(int p, int c) {
    p = p % getNumPalettes();
    c = c % palettes[p].length;
    return palettes[p][c];
  }

  static int[] getPalette(int p) {
    p = p % getNumPalettes();
    return palettes[p];
  }
}
