...
void main(void)
{
	...
	float dis = pow(distance(uv, vec2(0.5, 0.5)), 1.1)*0.24;
	...
	col = mix(col, vec4(0.0, 0.0, 0.0, 1.0), rand(uv+time)*0.1);
	col = mix(col, vec4(0.0, 0.0, 0.0, 1.0), pow(distance(uv, vec2(0.5, 0.5)), 2));
