class TouchManager {
  ...
  Touch click(float x, float y, float w, float h) {
    if (!click) return null;
    for (int i = 0; i < touchs.size (); i++) {
      Touch t = touchs.get(i);
      if (t.click && !t.used) {
        if (t.cx >= x && t.x <= x+w && t.cy >= y && t.cy < y+h) {
          t.used = true;
          return t;
        }
      }
