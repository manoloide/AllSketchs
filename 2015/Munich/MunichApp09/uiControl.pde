class UI {
  ...
  float atraccionTouch = 0.04;
  float distanciaDragueo = 400;
  float tiempoCell = 20;
  float tiempoTrend = 4;
  float tiempoTexto = 15;
  float tiempoAniTexto = 1.5;
  ...
  void createUI() {
    ...
    add(s6 = new Slider(10, 180, w-20, 20, 0, 3, tiempoTexto, "Tiempo Animacion Texto"));
  }
  ...
  void load() {
    ...
    if (f.exists()) {
      ...
      s1.setValue(json.getFloat(s1.name, atraccionTouch));
      ...
      s6.setValue(json.getFloat(s6.name, tiempoAniTexto));
      updateValues();
    } else {
      json = new JSONObject();
    }
