...
void render() {
  ...
  for (int i = 0; i < cc; i++) {
    ...
    float s = 800*(cos(time*TAU+da*i*4)*0.8+0.2);
    pushMatrix();
    translate(cos(a+time*TAU)*r, sin(a+da*i+time*TAU)*r);
    rotateX(time*TAU*1+(da*i)*0.5);
    rotateY(time*TAU*2+PI*0.5+(da*i));
    rotateZ(time*PI*6);
    float alp = 5;
    int col = getColor((time*2+i*(1.0/cc))*colors.length*2);
    col = lerpColor(col, color(255), 0.3);
    fill(col, alp);
    ellipse(0, 0, s*2, s*2);
    //sphere(s);
    popMatrix();
