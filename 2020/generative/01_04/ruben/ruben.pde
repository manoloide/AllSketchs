...
void gradColor() {
  ...
  renderCol.endDraw();
   //maskCol = createImage(render.width, render.height, ARGB);
  //arrayCopy(render.pixels, maskCol.pixels);
}
...
void render() {
  ...
  image(renderCol, 0, 0, width*0.2, height*0.2);
  image(renderSize, 0, height*0.2, width*0.2, height*0.2);
  //background(0);
  noStroke();
  for (int i = 0; i < 8000; i++) {
    ...
    int ix = int(xx);
    int iy = int(yy);
    int col = getColor();//renderCol.get(ix, iy);
