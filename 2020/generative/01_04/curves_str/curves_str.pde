...
void generate() {
  ...
  flower(width*0.5, height*0.5, width);

  /*
  for (int i = 0; i < 120; i++) {
   flower(width*random(1), height*random(1), random(width)*random(0.08, 0.4));
   }
   */
  /*
  for (int i = 0; i < 12; i++) {
   flower(random(width), random(height), random(width)*random(1));
   }
   */
}
...
void flower(float x, float y, float s) {
  ...
  //strokeWeight(1.4);
  noStroke();
  float strr = random(0.6, 0.8)*12.0;
  float ampD = 0.1;
  float oscZ = 1./(s/cc);
...
PVector def(float x, float y, float v) {
  ...
  float cx = width*0.5;
   ...
   float vv = constrain(dist(x, y, cx, cy)/260, 0, 1);
   x += cos(ang)*pow((vv), 2.4)*40;
   y += sin(ang)*pow((vv), 2.4)*40;
   */
