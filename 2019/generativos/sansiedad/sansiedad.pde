...
void generate() {
  ...
  float ic = random(100);
  float dc = random(0.08, 0.1)*0.4;
  float det = random(0.8, 0.1)*random(0.03, 0.04)*0.3;
  ...
  int div = int(random(0.8, 1)*15);
  for (int j = 0; j < div; j++) {
    float r1 = map(j+0, 0, div, 0, width*0.72);
    float r2 = map(j+1, 0, div, 0, width*0.72);
    int sub = int(random(0.6, 1)*2000);
    noFill();
    for (int i = 0; i < sub; i++) {
      float a1 = map(i+0, 0, sub, 0, TAU);
      stroke(getColor(noise(ic+dc*j)*colors.length*2), 16);
