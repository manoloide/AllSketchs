...
void generate() { 
  ...
  int grid = 80;
  int cw = int(width*1./grid);
  int ch = int(height*1./grid)+1;
  noFill();
  stroke(230);
  ...
  float sca = nwidth/960.;

  noStroke();
  for (int i = 0; i < max(cw, ch); i++) {

    int ww = int(random(1, cw*0.5));
    int hh = int(random(1, ch*0.5));

    int xx = int(random(1, cw-ww));
    int yy = int(random(1, ch-hh));
  ...
  for (int j = 0; j < max(cw, ch); j++) {
    float xx = int(random(1, cw-1))*grid;
    float yy = int(random(1, ch-1))*grid;
    fill(rcol());
