...
void generate() { 
  ...
  float det = random(0.0002, 0.0003)*0.4;

  float desCol = random(10000);
  float detCol = random(0.0004, 0.001);

  int cc = int(random(16, 18)*20);
  float ss = width*1./cc;
  
  blendMode(ADD);
  
  float lar = random(60, 80);

  noiseDetail(2);
  strokeWeight(random(0.4, 1.2));
  
  int trame = int(random(8, 20));
  for (int k = -5; k < cc+5; k++) {
    for (int j = -5; j < cc+5; j++) {
      ...
      stroke(getColor(noise(desCol+x*detCol, desCol+y*detCol)*10+((k+j)/trame)%2+((j/trame)%3)), 12);
