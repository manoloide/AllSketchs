...
void generate() { 
  ...
  noStroke();
  int cc = int(random(6, random(9, 12)));
  float ss = width*1./(cc);
  ...
  for (int i = 0; i < cc*cc*2; i++) {
    ...
    if (random(1) < 0.4) {
      ...
      float alp = random(110, 130);
      arc2(xx+dx, yy+dy, ss*0, ss*2, ang, ang+HALF_PI, rcol(), alp, 0);
      arc2(xx+dx, yy+dy, ss*2, ss*4, ang, ang+HALF_PI, rcol(), alp, 0);
    }
    ...
    if (random(1) < 0.5) {
      ...
      if (random(1) < 0.05) {
        ...
        arc2(xx, yy, ss*2, ss*4, HALF_PI*c, HALF_PI*(c+1), rcol(), random(30, 50), 0);
      }
      if (random(1) < 0.15) {
        fill(rcol()); 
        ellipse(xx+ddx, yy+ddy, ss, ss);
        arc2(xx+ddx, yy+ddy, ss, ss*2, 0, TAU, rcol(), random(30, 50), 0);
      }
      ...
      if (random(1) < 0.8) {
        ...
        //fill(rcol());
        //ellipse(xx, yy, s2*2, s2*2);
        fill(rcol());
        ellipse(xx, yy, s2, s2);
        //arc2(xx, yy, ss*0.2, ss*0.25, 0, TAU, color(255), random(30, 50), 0);
      }
      /*
      if (random(1) < 0.1) {
       fill(rcol());
