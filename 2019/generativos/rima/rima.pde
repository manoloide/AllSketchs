...
void generate() { 
  ...
  int sub = int(random(800, 1600)*random(0.6));
  for (int i = 0; i < sub; i++) {
  ...
  for (int i = 0; i < triangles.size(); i++) {
    ...
    noStroke();
    int ccc = 2+int(random(t.p1.dist(t.p2)+t.p2.dist(t.p3)+t.p3.dist(t.p1))*random(0.3)*random(1));
    if (sb) ccc = 2;
  ...
  for (int j = 0; j < points.size(); j++) {
    ...
    for (int i = 0; i < lar; i++) {
      stroke(lerpColor(getColor(ic+dc*i), color(0), cos(i*PI*0.1)*0.04+0.04));
      line(x1, y1, x2, y2); 
  ...
  for (int i = 0; i < 10; i++) {
    ...
    for (int j = 0; j < res; j++) {
      ...
      beginShape();
      fill(getColor(ic+d1*4));
      vertex(x1, y1);
      fill(getColor(ic+d2*4));
      vertex(x2, y2);
      vertex(cx, cy);
      endShape(CLOSE);
    }
