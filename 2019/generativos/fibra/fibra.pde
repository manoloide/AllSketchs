...
void generate() { 
  ...
  for(int i = 0; i < 8; i++){
     ...
     int sub = int(random(60, 200))*18;
     
     float ic = random(colors.length);
     float dc = random(0.004)*0.04;
     float da = random(PI);
     
     float amp = random(0.4);
     noStroke();
     noStroke();
     for(int j = 0; j < sub; j++){
        ...
        float ss = cos(v*j*0.2+xx*0.2)*250;
        stroke(getColor(noise(ic+dc*j)*colors.length*4), 80);
        PVector p = dis(xx, yy);
...
PVector dis(float xx, float yy) {
  ...
  float des = fbm(desDes+xx*detDes, seed+desDes+yy*detDes)*60;
