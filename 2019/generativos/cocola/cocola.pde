...
void generate() { 
  ...
  desAng = random(1000);
  detAng = random(0.006, 0.008)*0.3;
  desDes = random(1000);
  detDes = random(0.001, 0.0012)*0.5;

  float desRot = random(1000);
  float detRot = random(0.0008, 0.001);
  float desCol = random(1000);
  float detCol = random(0.006, 0.01)*0.4;
  float desMask = random(1000);
  float detMask = random(0.006, 0.01)*0.5;
  ...
  for (int i = 0; i < 300000; i++) {
    ...
    int ix = int(map(x, 0, width, mask.width*0.25, mask.width*0.74));
    int iy = int(map(y, 0, height, mask.height*0.1, mask.height*0.9));

    float v = 1.-brightness(mask.get(ix, iy))*1./255;

    v = 1-noise(desMask+x*detMask, desMask+y*detMask);
  ...
  for (int i = 0; i < 100000; i++) {
    ...
    float v = brightness(mask.get(ix, iy))*1./255;

    v = noise(desMask+x*detMask, desMask+y*detMask);
    ...
    //lar = 20;
    float alp = 120*pow(v, 0.9);
    stroke(getColor(noise(desCol+x*detCol, desCol+y*detCol)*5+random(1)+2), alp);
    noFill();
    beginShape();
    vline(x, y, x+dx, y+dy);
    endShape();
