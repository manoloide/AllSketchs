...
void generate() { 
  ...
  float detCol = random(0.02, 0.03)*0.12;
  ...
  for (int i = 0; i < 2200000; i++) {
    int x = int(random(cc));
    int y = int(random(cc));
    values[x][y] += pow(noise(des+x*det, des+y*det), 1.3)*0.5;
  }

  noiseDetail(2);
  for (int j = 0; j < cc; j++) {
    for (int i = 0; i < cc; i++) {
      float col = noise(desCol+j*detCol, desCol+i*detCol, noise(desCol+i*detCol, desCol+j*detCol)*40)*16;
      fill(lerpColor(getColor(col), color(255), 0.2));
      pushMatrix();
