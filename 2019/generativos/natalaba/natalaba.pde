...
void generate() { 
  ...
  */
  ...
  for (int i = 0; i < count; i++) {
    ...
    int cc = 200;//int(random(2000, 2800));
    ...
    for (int j = 0; j < cc; j++) {
      ...
      noStroke();
      strokeWeight(1);
      float amp3 = random(10)*random(0.5, 1);
      float amprad = pow(sin(j*PI/cc), 0.4)*10.2;
      int cccc = max(4, int(s*PI*0.1));
      float ddcc = random(1);
      for (int l = 0; l < cccc; l++) {
        fill(lerpColor(back, getColor(ic+dc*j+2+l*ddcc), 0.1+pow(i*1./count, 1.2)*0.8), 240);
        //fill(0, 40);
        for (int k = 0; k < ccc; k++) {
          float aa = ia+dd*k+da*j;
          float r = s*(1-rrr*0.5)*2.4*scale*amp3*map(l, 0, cccc, 1, 0);
