...
void draw() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    float a1 = random(TAU)+time*random(-1, 1)*0.004+time*random(-1, 1)*random(1.5)*random(1);
    float a2 = random(TAU)+time*random(-1, 1)*0.004+time*random(-1, 1)*random(1.5)*random(1);
    ...
    //beginShape(LINES);
    for (int j = 0; j < 500; j++) {
      a1 += ((float)SimplexNoise.noise(des1+xx*det1, des1+yy*det1, des1+zz*det1))*0.02;
      a2 += ((float)SimplexNoise.noise(des2+xx*det2, des2+yy*det2, des2+zz*det2))*0.02;
      xx = cos(a1)*cos(a2)*r;
      yy = cos(a1)*sin(a2)*r;
      zz = sin(a1)*r;
      vertex(xx, yy, zz);
    ...
    drawLine(points, random(20)*random(1));
  }
  //endShape();
}
