...
void generate() {
  ...
  for (int i = 0; i < div; i++) {
    ...
    for (int j = 0; j < sub; j++) {
      ...
      stroke(rcol());
      float ang = noise(xx*det, yy*det)*TAU*2;
      float x1 = xx+ww*0.5*cos(ang);
      float y1 = yy+hh*0.5*sin(ang);
      float x2 = xx+ww*0.5*cos(ang+PI);
      float y2 = yy+hh*0.5*sin(ang+PI);
      noiseLine(x1, y1, x2, y2, random(0.001));
    }
  }
}
...
void noiseLine(float x1, float y1, float x2, float y2, float det) {
  ...
  float ix = 0;
  float iy = 0;
  float dx = x1*0.0002;//random(1000);
  float dy = y1*0.0002;//random(1000);
