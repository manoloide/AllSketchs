...
void draw() {
  ...
  for (int j = -1; j < 2; j++) {
    for (int i = -1; i < 2; i++) {
      stroke(0, 40);
      bb(i*ww, j*hh, -200, ww*0.99, hh*.99, (ww+hh)*random(0.2, 0.05)*0.5, 4);
    }
  }
...
void bb(float x, float y, float z, float w, float h, float d, int ite) {
  if (ite < 0) return;
  ...
  noFill(); 
  d *= 0.5*map(cos(time*int(random(4))*PI+random(PI)), -1, 1, 0.8, 1.2);  
  d = max(d, 1);
  for (int i = 0; i < rects.size(); i++) {
    ...
    if(ite == 0) dd *= 10;
    pushMatrix();
    translate(r.x, r.y, z+dd*0.5);
    //if(ite == 0) 
    fbox(r.w, r.h, dd);
    popMatrix();
    
    
    bb(r.x, r.y, z, r.w, r.h, d, ite-1);
  }
}
