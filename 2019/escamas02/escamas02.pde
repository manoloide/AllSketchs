...
void generate() { 
  ...
  time += cos(time*random(0.1))*random(0.001)*random(1);
  ...
  for (int j = 0; j < cc; j++) {
    for (int i = 0; i < cc; i++) {
      ...
      int sprite = (int) (SimplexNoise.noise(xx*detSpri, yy*detSpri, time*0.8)*0.5+0.5)*10;
      ...
      scale(pow((float) SimplexNoise.noise(xx*det, yy*det, time)*0.9+0.1, 1.4)*2);
      //rect(00, 00, dd*0.6, dd*0.6);
      //ellipse(00, 00, dd*0.6, dd*0.6);
      tint((type == 0)? c1 : c2, random(255));
