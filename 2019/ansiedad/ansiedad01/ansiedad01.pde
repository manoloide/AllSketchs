...
void generate() {
  ...
  float det = random(0.8, 0.1)*random(0.03, 0.04)*0.1;
  ...
  int div = int(random(0.8, 1)*14*random(1, 2));
  float da = random(20);
  for (int j = 0; j < div; j++) {
    float r1 = pow(map(j+0+cos(j*da)*4, 0, div, 0, 1), pwr)*width*0.82;
    float r2 = pow(map(j+random(1, 2)+cos(j*da)*4, 0, div, 0, 1), pwr)*width*0.82;
    int sub = int(r2*PI*random(0.6, 1)*random(6));
    noFill();
    for (int i = 0; i < sub; i++) {
      float a1 = map(i+0, 0, sub, 0, TAU);
      stroke(getColor(noise(ic+dc*j)*colors.length*2+random(0.2)), random(10, random(10, 20))*3 );
