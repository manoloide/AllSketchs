...
void setup() {
  ...
  image = loadImage("72171592_2603941786331980_7696821916895543296_n.jpg");
...
void draw() {
  ...
  for (int i = 0; i < sub; i++) {
    ...
    float det1 = random(0.03);
    float det2 = random(0.03);
    ...
    float ss = 60*(0.5+val)*0.08;

    int col = getColor((val+nt*8.0)*colors.length);
    int x = int(map(xx, 0, width, 0, image.width));
    int y = int(map(yy, 0, height, 0, image.height));
    col = image.get(x, y);
    //col = color(0);

    imageMode(CENTER);
    tint(col, random(80, 100)*2.4);
  ...
  if (export) {
    if (frameCount >= timeExport*30) saveFrame("export/f"+nf(int(frameCount-(timeExport*30)), 4)+".png");
...
int colors[] = {#F7513B, #9DAAAB, #6789AA, #4B4372, #262626};
