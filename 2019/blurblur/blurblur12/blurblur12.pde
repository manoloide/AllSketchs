...
int seed = 1200;//int(random(99999999));
...
void draw() {
  ...
  for (int i = 0; i < sub; i++) {
    ...
    float xx = (float) (SimplexNoise.noise(1, det*i, tt)+0.5)*width;
    float yy = (float) (SimplexNoise.noise(det*i, 23, tt)+0.5)*height;
    ...
    boolean sprite = true;
    float ss = s*(1-val)*(0.2+cos(time+i)*0.2)*12;
