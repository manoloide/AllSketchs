...
int seed = 80;//int(random(99999999));
...
void draw() {
  ...
  for (int i = 0; i < sub; i++) {
    ...
    float s = max(abs(cos(time*0.71+i)*sin(time)), 0.3)*width*random(0.008, 0.02)*2;
    ...
    int col = getColor(time*random(20)+random(200));
    ...
    if (sprite) {
      ...
      float sss = ss*(0.6+cos(time*random(100))*0.4)*3;

      image(text, xx, yy, sss, sss);
      tint(255, 90);
