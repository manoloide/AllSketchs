...
void main(void){
    ...
    vec2 nd = direction*noise(st*60.0+cos(time*PI*50.0))*0.6;//1.0;//*noise(vertTexCoord.xy*0.1+vec2(0.0, time));
    
    float r = pow(blur13(texture, st, nd*3.1).r, 0.999);//*1.00001;
    float g = blur13(texture, st, nd*3.1).g*0.98;
    float b = blur13(texture, st, nd*2.5).b;
    
    float blur = 1.01;
    
    r = mix(r, col.r, 0.8*blur);
    g = mix(g, col.g, 0.9*blur);
    b = mix(b, col.b, .99*blur);
    
    
    gl_FragColor = vec4(r, g, b, 1.0) * 1.002 * vertColor;
