...
int nwidth = 8268;//8268; //960;
int nheight = 8268;//8268; //960;
int swidth = 960; 
int sheight = 960;
int twidth = 2067;//960*2; 
int theight = 2067;//960*2;
float scale = 1;
  ...
  for (int k = -10; k < cc+10; k++) {
    for (int j = -10; j < cc+10; j++) {
      ...
      if (points.size() > 1 && in) {
        ...
        float nc = noise(desCol+p.x*detCol, desCol+p.y*detCol)*50;
        float velCol = noise(desvc+p.x*detvc, desvc+p.y*detvc)*0.01;//random(0.01)*random(1);
        ...
        for (int i = 0; i < points.size(); i++) {
          ...
          nc += velCol;
          //velCol *= modVelCol;
          render.stroke(getColor(curve(nc%1, 2)*colors.length), 170);
