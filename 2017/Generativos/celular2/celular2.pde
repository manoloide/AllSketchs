...
void generate() {
  ...
  for (int i = 0; i < 100000; i++) {
    ...
    float ss = random(5, random(100, 600))*random(0.2, 1);
...
class Line {
  ...
  void show() {
    ...
    int res = 10;
    float r = ss*0.5;
    PVector[] aux = new PVector[res*2];
    noStroke();
    fill(getColor(random(colors.length)));
    beginShape();
    for (int i = 0; i < aux.length; i++) {
      aux[i] = (i < res)? p2.copy() : p1.copy();
      float ang = a+map(i, 0, aux.length-1, -PI*0.5, PI*1.5);
      aux[i].add(new PVector(cos(ang)*r, sin(ang)*r));
      vertex(aux[i].x, aux[i].y);
    }
    endShape();
  }
