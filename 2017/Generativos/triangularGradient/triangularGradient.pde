...
void render() {
  ...
  for (int i = 0; i < tris.size(); i++) {
    ...
    PVector c = circleCenter(t.p1, t.p2, t.p3);
    float ss = dist(c.x, c.y, t.p1.x, t.p1.y)*2;
    ...
    /*
    float c1 = noise(t.p1.x*det, t.p1.y*det)*5*colors.length;
     float c2 = noise(t.p2.x*det, t.p2.y*det)*5*colors.length;
     float c3 = noise(t.p3.x*det, t.p3.y*det)*5*colors.length;
     float s1 = 1+noise(34.2+t.p1.x*det, 56.1+t.p1.y*det)*3;
     float s2 = 1+noise(34.2+t.p2.x*det, 56.1+t.p2.y*det)*3;
     float s3 = 1+noise(34.2+t.p3.x*det, 56.1+t.p3.y*det)*3;
     line(t.p1.x, t.p1.y, t.p2.x, t.p2.y, s1, s2, c1, c2); 
     line(t.p2.x, t.p2.y, t.p3.x, t.p3.y, s2, s3, c2, c3); 
     line(t.p3.x, t.p3.y, t.p1.x, t.p1.y, s3, s1, c3, c1);
     */
  }
...
PVector circleCenter(PVector p1, PVector p2, PVector p3) {
  ...
  c.x = (aSlope*bSlope*(p1.y - p3.y) + bSlope*(p1.x + p2.x)
    - aSlope*(p2.x+p3.x) )/(2* (bSlope-aSlope) );
  c.y = -1*(c.x - (p1.x+p2.x)/2)/aSlope +  (p1.y+p2.y)/2;
